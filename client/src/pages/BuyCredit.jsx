...
const BuyCredit = () => {
    ...
    const initPay = async (order)=>{
        const options ={
            ...
            description: 'Pay to Get Your Credits',
            order_id: order.id,
    ...
    const paymentRazorpay = async (planId) =>{
        try{
            const token = await getToken()
            const {data} = await axios.post(backendUrl+'/api/user/pay-razor',{planId},{headers:{token}})        //JWT is passed in the headers
            if (data.success){
                initPay(data.order)
            }
        }
    ...
    <div className='min-h-[80vh] text-center pt-14 mb-10'>
        ...
        <div className='flex flex-wrap justify-center gap-6 text-left'>
            {plans.map((item,index)=>(
                <div className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-700 hover:scale-105 duration-500' key={index}>
